...
waste_audit <- waste_audit %>% 
  mutate(perc_missorted = case_when(original_stream == "Trash" ~ (pieces_recycle + pieces_compost) / total_piece_volume, original_stream == "Recycling" ~ (pieces_trash + pieces_compost) / total_piece_volume))
```
...
ci_boot_missorted <- get_ci(boot_missorted, level = 0.99)
ci_boot_missorted
```
...
this represents the amount of random sampling error in a study; bigger indicates little confidence in generalizability of data

How does this impact what our values mean? Can this percentage be applied to create a new range of results from the audit?
